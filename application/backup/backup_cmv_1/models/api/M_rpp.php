...
class M_rpp extends CI_Model
{
	...
	public function rpp_result()
	{
		...
		$this->db->from('rpp');
		$this->db->join('kelas', 'kelas.id = rpp.id_kelas', 'left');

		if ($search != null) {
			$this->db->group_start();
			$this->db->like('rpp.nama_guru', $search);
			$this->db->or_like('rpp.judul', $search);
			$this->db->group_end();
		}

		if ($level != 'Admin') {
			$this->db->where('rpp.id_guru', $id_guru);
		}

		$this->db->order_by('rpp.id', 'DESC'); // 🔽 data terbaru di atas
