	...
	public function print_laporan()
	{
		...
		if ($ambil['filter'] == 'tanggal') {
			...
			if ($pegawai == null) {
				...
				$jurnal_pegawai_rows = $this->db->query($sql, [$dari_tanggal, $sampai_tanggal])->result_array();


				$group_nama = [];
				foreach ($jurnal_pegawai_rows as $row) {
					$tgl = $row['nama_pegawai'];
					if (!isset($group_nama[$tgl]))
						$group_nama[$tgl] = [];
					$group_nama[$tgl][] = $row;
				}
				$jurnal_pegawai = $group_nama;
			...
			if ($pegawai == null) {
				...
				$jurnal_pegawai_rows = $this->db->query($sql, [$bulan, $tahun])->result_array();
			...
			if ($pegawai == null) {
				...
				$jurnal_pegawai_rows = $this->db->query($sql, [$tahun])->result_array();

				$group_tgl = [];
				foreach ($jurnal_pegawai_rows as $row) {
					$tgl = $row['tanggal'];
					if (!isset($group_tgl[$tgl]))
						$group_tgl[$tgl] = [];
					$group_tgl[$tgl][] = $row;
				}
				$jurnal_pegawai = $group_tgl;
			} else {
