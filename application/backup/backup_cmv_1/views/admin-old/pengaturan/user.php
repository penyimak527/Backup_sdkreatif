<div class="card">
	...
	<div class="card-body">
		...
		<div class="table-responsive-sm">
			<table class="table table-bordered m-b-0" id="table_user">
				<thead>
					<tr>
...
<script>
	$(document).ready(function () {
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#table_user tbody tr'), jumlah);
		});
	})
	...
	function user() {
		...
		$.ajax({
			...
			success: function (data) {
				...
				`;
					...
					data.forEach(function (item) {
						...
						<tr>
							...
							<td>${item.username}</td> 
							<td>${item.level}</td> 
							<td> <a href="<?= base_url('admin/pengaturan/user/view/'); ?>${item.id}" class="btn btn-sm btn-outline-warning"><i class="ri-edit-2-line"></i></a>
				...
				$('#table_user tbody').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#table_user tbody tr'), jumlah_awal);
			}
		});
	}
