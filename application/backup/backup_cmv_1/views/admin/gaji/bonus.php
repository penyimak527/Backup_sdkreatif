	...
	<div class="card-body">
		<div class="row">
			<div class="col-md-3">
				<div class="mb-3">
					<div class="input-group">
						<input type="text" class="form-control" id="cari-pegawai" placeholder="Cari Pegawai"
							aria-describedby="inputGroupPrepend" onkeyup="bonus()">
                ...
                <div class="mb-3">
                    <div class="input-group">
                        <select id="bulan-bonus" class=" form-control ">
            ...
            <div class="col-md-2">
                <div class="mb-3">
                    <div class="input-group">
                        <select id="tahun-bonus" class=" form-control ">
            ...
            <div class="col-md-3">
                <div class="mb-3">
                    <button type="button" class="btn btn-sm btn-primary" onclick="bonus()"><i
				...
				<form id="form-tambah">
					...
					<div class="row mb-3">
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" data-width="100%" name="bulan_bonus">
									<?php
									$bulan = array("Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember");
									$jlh_bln = count($bulan);
									$no = 0;
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" data-width="100%" name="tahun_bonus">
									<?php
									$now = date('Y');
									for ($a = 2025; $a <= $now; $a++) {
										?>
					...
					<div class="mb-3">
						...
						<input type="text" name="nominal" class="form-control" placeholder="Nominal ..."
							onkeyup="FormatCurrency(this);" /></div>
					</div>
				</form>
			</div>
...
<script>
	$(document).ready(function () {
		...
		$("#btn-simpan").click(function () {
			...
			$.ajax({
				url: '<?= base_url('admin/gaji/bonus/tambah'); ?>',
				type: 'POST',
				data: formData,
				dataType: 'JSON',
		...
		$("#btn-update").click(function () {
			...
			var formData = form.serialize();
			$.ajax({
				url: '<?= base_url('admin/gaji/bonus/edit'); ?>',
				type: 'POST',
				data: formData,
				dataType: 'JSON',
				success: function (data) {
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#data_bonus .card-mapel'), jumlah);
		});
	})
	function pegawai(id_pegawai = null) {
		$.ajax({
			url: '<?= base_url('admin/gaji/bonus/pegawai_result'); ?>',
			type: 'POST',
			dataType: 'JSON',
			success: function (data) {
				var no = 1;
 ...
 function namaBulan(bulan) {
	...
	function bonus() {
		...
		var bulan = $('#bulan-bonus').val();
		var tahun = $('#tahun-bonus').val();
		$.ajax({
			url: '<?= base_url('admin/gaji/bonus/bonus_result'); ?>',
			type: 'POST',
			data: {
				search: cari,
				bulan: bulan,
			...
			success: function (data) {
					...
					data.forEach(function (item) {
						...
						table += `
							...
							<div class="card-mapel">
							  <p class="keterangan-hari">Bulan : ${namaBulan(item.bulan)} ${item.tahun}</p>
							<div class="keterangan-mapel">
								<div class="keterangan-mapel-kiri">
									<h5 class="judul-mapel" style="margin:0; margin-top: 8px;">${no++}. ${item.nama_pegawai}</h5>   
									  ...
									  <p class="keterangan-jam-mapel">Bonus : Rp. ${NumberToMoney(item.nominal)}</p>
								</div>
								 <div class="keterangan-mapel-kanan">
									<div class="d-flex justify-content-center gap-2">
										<button type="button" onclick="editBonus('${detail}')" class="btn btn-sm btn-outline-warning me-1"><i class="ri-edit-2-line"></i></button>
				...
				$('#data_bonus').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#data_bonus .card-mapel'), jumlah_awal);
			}
		});
	}

	function editBonus(detail) {
		$('#edit').modal('show');
		var item = JSON.parse(atob(detail));
		$('#edit input[name="id_bonus"]').val(item.id);
		$('#edit select[name="bulan_bonus"]').val(item.bulan);
		$('#edit select[name="tahun_bonus"]').val(item.tahun);
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: `<?= base_url(); ?>admin/gaji/bonus/hapus`,
					data: {
						id: id
					},
					dataType: 'json',
