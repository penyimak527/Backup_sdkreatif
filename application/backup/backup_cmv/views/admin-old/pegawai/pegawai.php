<div class="card">
	...
	<div class="card-body">
		...
		<div class="table-responsive-sm">
			<table class="table table-bordered m-b-0" id="table_pegawai">
				<thead>
					<tr>
...
<script>
	$(document).ready(function () {
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#table_pegawai tbody tr'), jumlah);
		});
	})
	...
	function pegawai() {
		$.ajax({
			...
			success: function (data) {
				...
				`;
					...
					data.forEach(function (item) {
						...
						<tr>
							...
							<td>${item.no_tlp}</td> 
							<td> <a href="<?= base_url('admin/pegawai/pegawai_jabatan/view/'); ?>${item.id}" class="btn btn-sm btn-outline-warning"><i class="ri-edit-2-line"></i></a>
				...
				$('#table_pegawai tbody').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#table_pegawai tbody tr'), jumlah_awal);
			}
		});
	}
	...
	function paging($selector, jumlah_tampil = 10) {
		if (typeof $selector === 'undefined') {
			$selector = $("#table_pegawai tbody tr");
		}
