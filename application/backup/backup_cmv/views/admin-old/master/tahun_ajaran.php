<div class="card">
	...
	<div class="card-body">
		...
		<div class="table-responsive">
			<table class="table table-bordered m-b-0" id="table_tahun_ajaran">
				<thead>
					<tr>
...
<script>
	$(document).ready(function () {
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#table_tahun_ajaran tbody tr'), jumlah);
		});
	})
	...
	function tahun_ajaran() {
		...
		$.ajax({
			...
			success: function (data) {
				...
				`;
					...
					data.forEach(function (item) {
						...
						<tr>
							...
							<td>${item.periode}</td>
							<td><span class="badge bg-${item.status == 'Aktif' ? 'success' : 'danger'}">${item.status}</span> </td>
							<td>
								<button type="button" class="btn btn-sm btn-outline-primary " onclick="update_status(${item.id})"><i class="ri-file-warning-line"></i></button>
								<button type="button" class="btn btn-sm btn-outline-warning " onclick="editTahun('${detail}')"><i class="ri-edit-2-line"></i></button>
				...
				$('#table_tahun_ajaran tbody').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#table_tahun_ajaran tbody tr'), jumlah_awal);
			}
		});
	}
	...
	function paging($selector, jumlah_tampil = 10) {
		if (typeof $selector === 'undefined') {
			$selector = $("#table_tahun_ajaran tbody tr");
		}
