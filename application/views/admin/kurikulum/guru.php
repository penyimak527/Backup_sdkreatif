...
<script>
	$(document).ready(function () {
		$('#dt-length-0').on('change', function () {
			...
			paging($('#data_guru .card-mapel'), jumlah);
		});
		guru()
	})
	...
	function guru() {
		...
		$.ajax({
			...
			success: function (data) {
					...
					data.forEach(function (item) {
						...
						<div class="card-mapel">
							<p class="keterangan-hari">
								<span>NBM : ${item.nbm}</span> 
								<span>No Telp : ${item.no_telp}</span> 
							</p>
							<div class="keterangan-mapel">
								<div class="keterangan-mapel-kiri">
									<h5 class="judul-mapel" style="margin:0; margin-top: 8px;">${no++}. ${item.nama_guru}</h5> 
									<span style="margin: 0; padding: 0; font-size: 12px; margin-bottom: 8px;">Jenis Kelamin : ${item.jk}</span>
									<p class="d-flex gap-2 m-0 mb-1"> 
										<span>TTl: ${item.tempat_lahir}, ${item.tanggal_lahir}</span>
									</p> 
									<p class="keterangan-jam-mapel">Mata Pelajaran :  ${item.mapel.map(m => `<span class="badge bg-primary me-1" style="font-size: 12px;">${m.mapel}</span>`).join('')}</p>
								</div>
				...
				$('#data_guru').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#data_guru .card-mapel'), jumlah_awal);
			}
		});
	}
