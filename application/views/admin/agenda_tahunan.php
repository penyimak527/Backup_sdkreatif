		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-tambah" enctype="multipart/form-data">
					...
					<?php if ($level == 'Admin'): ?>
						<div class="mb-3">
							<label for="id_pegawai" class="form-label">Penanggung Jawab</label>
...
<script>
						...
						<div class="card-mapel">
							...
							<div class="keterangan-mapel">
								<div class="keterangan-mapel-kiri">
									...
									<p class="keterangan-jam-mapel">Penanggung Jawab : ${item.pegawai}</p>
									<p class="keterangan-jam-mapel">Status : ${item.status}</p>
								</div>
	...
	function viewAgenda(detail) {
		...
		if (item.file == '') {
			...
			$('#data-agenda_tahunan').html(`
				  <iframe src="<?= base_url(); ?>storage/agenda_tahunan/${item.file}" 
			  width="100%" 
			  height="600px"  ></iframe>`);
		}
	}
	...
	function pegawai(id_pegawai = null) {
		$.ajax({
			url: '<?= base_url('admin/agenda_tahunan/pegawai_result'); ?>',
			type: 'POST',
			dataType: 'JSON',
			success: function (data) {
