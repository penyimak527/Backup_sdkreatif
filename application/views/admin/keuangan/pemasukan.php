	...
	<div class="card-body">
		<div class="row">
				...
				<div class="mb-3">
					<div class="input-group">
						<input type="text" class="form-control" id="cari-keterangan" placeholder="Cari Keterangan"
							aria-describedby="inputGroupPrepend" onkeyup="pemasukan()">
			...
			<div class="col-md-2">
				<div class="mb-3">
					<div class="input-group">
						<select id="cari-pemasukan-keterangan" class=" form-control select2-normal">
			...
			<div class="col-md-3">
				<div class="mb-3">
					<button type="button" class="btn btn-sm btn-primary" onclick="pemasukan()"><i
		...
		<div class="table-responsive">
			<table class="table table-bordered table-hover table-sm" id="table_pemasukan">
				<thead>
					<tr>
						<th style="text-align: center;" width="60">No</th>
						<th width="120" onclick="urutTanggal()" style="cursor:pointer">Tanggal <i
								class="ri-arrow-up-down-line"></i></th>
						<th width="150">Kode Akun</th>
						<th width="150">Nominal</th>
						<th width="250">Keterangan</th>
						<th width="150">Simpan Ke</th>
					...
					<tr>
						...
						<th class="align-top">
							<select id="id_simpan_ke" class="form-control form-control-sm">
						...
						<th class="align-top">
							<button class="btn btn-primary btn-sm" style="height: 30px; width: 60px;" id="btn-simpan"
								onclick="simpanBaru()">
								Simpan
							</button>
						</th>
...
<script>
	$(document).ready(function () {
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#table_pemasukan tbody tr'), jumlah);
		});
		$('.select2').select2({
			width: '100%'
		});
	...
	function simpanBaru() {
		...
		formData.append('id_simpan_ke', $('#id_simpan_ke').val());
		let file = $('#bukti_tambah')[0].files[0];
		if (file) {
			formData.append('bukti', file);
		}
		$.ajax({
			url: '<?= base_url('admin/keuangan/pemasukan/tambah') ?>',
			type: 'POST',
			data: formData,
			contentType: false,
			processData: false,
	...
	function urutTanggal() {
		...
		pemasukan(sortTanggal);
	}
	function pemasukan(sortTanggal = null) {
		var keterangan = $('#cari-keterangan').val();
		var kategori = $('#cari-pemasukan-keterangan').val();
		var tanggal_dari = $('input[name="tanggal-cari-dari"]').val();
		var tanggal_sampai = $('input[name="tanggal-cari-sampai"]').val();
		$.ajax({
			url: '<?= base_url('admin/keuangan/pemasukan/pemasukan_result'); ?>',
			type: 'POST',
			data: {
			...
			<tr id="row_${item.id}">
				...
				<td class="id_simpan_ke" data-value="${item.simpan_ke}">
					${item.nama_kas ?? '-'}
				</td>
				<td>
					 ${item.bukti ? `<img src="<?= base_url(); ?>${item.bukti}" class="img-preview" onclick="previewImage('<?= base_url(); ?>${item.bukti}')">` : '-'}
				</td>
				<td>
					<button class="btn btn-sm btn-warning" style="width: 30px; height: 30px;"  onclick="editpemasukan('${no}','${item.id}','${detail}')">
				...
				$('#table_pemasukan tbody').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#table_pemasukan tbody tr'), jumlah_awal);
	...
	let originalRows = {};
	function editpemasukan(no, id, detail) {
		...
		<td>
			<select id="simpan_ke_edit_${id}" class="form-control form-control-sm select2-edit">
				<option value="">Pilih Kas Bank</option>
...
<td>
	${item.bukti ? ` <div id="preview_container_${id}">
	...
	`);
		...
		$('#simpan_ke_edit_' + id).val(item.simpan_ke);
	...
	function simpanEdit(id) {
		let formData = new FormData();
		formData.append('id_pemasukan', id);
		...
		formData.append('id_simpan_ke', $('#simpan_ke_edit_' + id).val());
		...
		$.ajax({
			url: '<?= base_url("admin/keuangan/pemasukan/edit") ?>',
			type: 'POST',
			data: formData,
			processData: false,
			contentType: false,
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: `<?= base_url(); ?>admin/keuangan/pemasukan/hapus`,
					data: {
						id: id
					},
					success: function (data) {
