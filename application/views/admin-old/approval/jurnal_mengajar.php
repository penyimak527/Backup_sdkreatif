		...
		$.ajax({
			...
			success: function (data) {
				...
				`;
					...
					// Object.entries(data).forEach(([tanggal, data_per_guru]) => {
					// 	table += `
					// 	<div class="card mb-4">
					// 		<div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
					// 			<h4 class="header-title">Tanggal ${tanggal}</h4>
					...
					// 	Object.entries(data_per_guru).forEach(([id_guru, jurnal_items]) => {
					...
					// 				<span>Tahun Ajaran: ${item.periode}</span>
					// 				<span>
					// 					<input type="checkbox" id="cek_tanggal-${item.id}-${no}" name="id_jurnal[]" value="${item.id}">
					// 				</span>
					// 				<input type="hidden" name="tipe[]" value="${item.tipe}">
					// 			</p>
					// 			`;
					// 		jurnal_items.forEach(item => {
					// 			table += `
					// 				<div class="keterangan-mapel">
					// 				<div class="keterangan-mapel-kiri">
					// 					<h5 class="judul-mapel">${item.nama_guru}</h5>
					...
					// 		// 	if (item.tipe === 'pegawai') {
					// 		// 		tipe = `< p p class="keterangan-jam-mapel" > Kegiatan: ${ item.jam_selesai_pelajaran }</p > `;
					// 		// 		detail = `< button button type = "button" class="btn btn-sm btn-primary w-100" onclick = "persetujuan_pegawai('${item.id}')" > <i class="ri-eye-line" style="font-size: 20px;"></i> Detail</button > `;
					// 		// 	} else {
					// 		// 		tipe = `< p p class="keterangan-jam-mapel" > Waktu Mengajar: ${ item.jam_mulai_pelajaran } - ${ item.jam_selesai_pelajaran }</p > `;
					// 		// 		detail = `< button button type = "button" class="btn btn-sm btn-primary w-100" onclick = "persetujuan('${item.tanggal}', ${item.id_guru}, '${item.nama_guru}')" > <i class="ri-eye-line" style="font-size: 20px;"></i> Detail</button > `;
					// 		// 	}
				...
				data.forEach(function (item) {
					...
					<div class="jurnal-box">
								<div class="jurnal-item">
									...
									<div class="jurnal-value">${item.nama_kelas}</div>
								</div>
