...
class Laporan_presensi_pegawai extends CI_Controller
{
	...
	public function print_laporan()
	{
		...
		if ($ambil['filter'] == 'tanggal') {
			...
			if ($tampil == 'tampil' || $tampil == '') {
				...
				for ($current = $start; $current <= $end; $current = strtotime('+1 day', $current)) {
					...
					foreach ($pegawai_list as $pegawai) {
						$presensi = $this->db->query("SELECT * FROM presensi_pegawai WHERE id_pegawai = ? 
						AND tanggal = ? ", [$pegawai['id_pegawai'], $tanggal])->row_array();
			...
			$grouped_by_tanggal = [];
			$pegawai_list = $this->db->query("SELECT * FROM pegawai_jabatan ORDER BY id asc")->result_array();
			$presensi_raw = $this->db->query("SELECT id_pegawai, tanggal, waktu, jam_terlambat, jam_pulang, status FROM presensi_pegawai 
			WHERE tanggal LIKE '%-$tahun'")->result_array();
			$presensi_index = [];
