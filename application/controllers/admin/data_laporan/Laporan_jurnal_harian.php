	...
	public function print_laporan()
	{
		...
		if ($ambil['filter'] == 'tanggal') {
			...
			left join kelas_jadwal_pelajaran b on a.id_kelas_jadwal_pelajaran = b.id
			 ...
			 WHERE STR_TO_DATE(a.tanggal, '%d-%m-%Y') >= STR_TO_DATE('$dari_tanggal', '%d-%m-%Y') AND STR_TO_DATE(a.tanggal, '%d-%m-%Y') <= STR_TO_DATE('$sampai_tanggal', '%d-%m-%Y')  AND a.id_guru = '$guru' AND a.id_periode = '$periode' AND a.semester = '$semester'")->result_array();
			...
			left join kelas_jadwal_pelajaran b on a.id_kelas_jadwal_pelajaran = b.id
				 left join kelas d on b.id_kelas = d.id
			 WHERE MONTH(STR_TO_DATE(a.tanggal, '%d-%m-%Y')) = '$bulan' AND YEAR(STR_TO_DATE(a.tanggal, '%d-%m-%Y')) = '$tahun' AND a.id_guru = '$guru' AND a.id_periode = '$periode' AND a.semester = '$semester'")->result_array();
			...
			left join kelas_jadwal_pelajaran b on a.id_kelas_jadwal_pelajaran = b.id
				 left join kelas d on b.id_kelas = d.id
			 WHERE  YEAR(STR_TO_DATE(a.tanggal, '%d-%m-%Y')) = '$tahun' AND a.id_guru = '$guru' AND a.id_periode = '$periode' AND a.semester = '$semester'")->result_array();
