			...
			<div class="modal-body">
				<form id="form-tambah" enctype="multipart/form-data">
					...
					<div class="mb-3">
						...
						<input type="file" name="file_rpp[]" class="dropify" placeholder="Judul ..." multiple />
						<span style="color: brown;">Pastikan internet stabil agar tidak menghambat upload file</span>
					</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-edit" enctype="multipart/form-data">
					...
					<div class="mb-3">
						<label for="file_rpp" class="form-label">Upload Rpp</label>
						<input id="edit-rpp" type="file" name="file_rpp[]" class="form-control" multiple>
					</div>
...
<div class="modal fade" id="view-rpp" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="myLargeModalLabel">Dokumen <?= $title; ?></h4>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body">
				<div class="row mb-3" id="data-rpp">
...
<script>
						...
						<div class="card-mapel">
							...
							<div class="keterangan-mapel">
								<div class="keterangan-mapel-kiri">
									...
									<p class="keterangan-jam-mapel">Kelas : ${item.kelas} ${item.kode_kelas}</p>
								</div>
								<div class="keterangan-mapel-kanan-custom">
									<button type="button" class="btn btn-sm btn-outline-primary " onclick="viewRpp('${item.id}')"><i class="ri-eye-line"></i></button>
	...
	function viewRpp(id_rpp) {
		...
		if (id_rpp == '') {
			...
			$.ajax({
				type: 'POST',
				url: `<?= base_url(); ?>admin/rpp/file_rpp`,
				data: {
					id_rpp
				},
				dataType: 'json',
					...
					if (result.status == true) {
						result.data.forEach(function (item) {
							file += `
								...
								<div class="mb-1">
									<div class="input-group">
										<input type="text" class="form-control" id="cari-rpp" value="${item.file}" disabled>
										<a href="<?= base_url(); ?>storage/guru/rpp/${item.file}" class="input-group-text bg-primary text-white" target="blank"> <i class="ri-file-pdf-2-fill"></i></a>
									</div>
								</div>
							</div>
							`;
