...
class M_jurnal_guru extends CI_Model
{
	...
	public function riwayat_mengajar_result()
	{
		...
		$this->db->order_by("STR_TO_DATE(jurnal_guru.tanggal_input, '%d-%m-%Y') DESC");
		$query = $this->db->get();
	...
	public function jurnal_guru_result()
{
    ...
    foreach ($data_mengajar as &$item) {
        $tanggal = $this->getTanggalHariSemester(
            ...
            $semester_aktif,
			$periode_aktif['tanggal_awal_semester_ganjil'],
			$periode_aktif['tanggal_awal_semester_genap'],
			$periode_aktif['tanggal_akhir_semester_ganjil'],
			$periode_aktif['tanggal_akhir_semester_genap']
        );
...
public function getTanggalHariSemester($hari, $periode, $semester, $tanggal_awal_semester_ganjil, $tanggal_awal_semester_genap, $tanggal_akhir_semester_ganjil, $tanggal_akhir_semester_genap)
{
    $hariArray = [
        'Senin' => 'monday',
        'Selasa' => 'tuesday',
    ...
    if ($semester == 'Ganjil') {
		$tanggal_awal = $tanggal_awal_semester_ganjil == null ? $tahun_awal . '-07-01' : $tanggal_awal_semester_ganjil;
		$tanggal_akhir = $tanggal_akhir_semester_ganjil == null ? $tahun_awal . '-12-31' : $tanggal_akhir_semester_ganjil;
	} else {
		$tanggal_awal = $tanggal_awal_semester_genap == null ? $tahun_akhir . '-01-01' : $tanggal_awal_semester_genap;
		$tanggal_akhir = $tanggal_akhir_semester_genap == null ? $tahun_akhir . '-06-30' : $tanggal_akhir_semester_genap;
	}
	...
	public function tambah()
	{
		...
		// 	}
		// }
		$periode = $this->db->get_where('master_tahun_ajaran', ['status' => 'Aktif'])->row_array();
