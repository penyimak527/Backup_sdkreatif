...
class M_jurnal_guru extends CI_Model
{
	...
	public function riwayat_mengajar_result()
	{
		...
		$this->db->select('jurnal_guru.*, kelas_jadwal_pelajaran.hari, kelas_setting.nama_kelas');
	...
	public function jadwal_mengajar_result()
	{
		...
		$this->db->select('kelas_jadwal_pelajaran.*,   kelas_setting.*');
	...
	public function jurnal_guru_result()
	{
		...
		JOIN master_mata_pelajaran b ON a.id_mapel = b.id
		JOIN guru c ON a.id_guru = c.id
		JOIN kelas_setting d ON a.id_kelas_setting = d.id
		JOIN kelas e ON d.id_kelas = e.id
		JOIN master_tahun_ajaran f ON d.id_periode = f.id
	...
	")->result_array();
		...
		foreach ($data_mengajar as &$item) {
			...
			$tahun = date('Y');
			if ($tahun == 2025) {
				$tanggal = $this->getTahunSementara($item['hari']);

			} else {
				$tanggal = $this->getTanggalHariSetahun($item['hari']);
			}
		...
		foreach ($data_mengajar as $jadwal_item) {
			...
			if (is_array($jadwal_item['tanggal'])) {
				foreach ($jadwal_item['tanggal'] as $tgl) {
					...
					// Lewati jika tanggal lebih dari hari ini
					if ($tgl_string > date('Y-m-d')) {
						continue;
					}

					if (date('d-m-Y', strtotime($tgl)) == !empty($tgl_jurnal_guru['tanggal'])) {
						continue;
					}
	...
	public function tambah()
	{
		...
		$this->db->select('kelas_jadwal_pelajaran.*, kelas_setting.semester,kelas_setting.id_periode,kelas_setting.periode'); // sesuaikan kolom yang dibutuhkan
		$this->db->from('kelas_jadwal_pelajaran');
		$this->db->join('kelas_setting', 'kelas_setting.id = kelas_jadwal_pelajaran.id_kelas_setting'); // sesuaikan kolom foreign key-nya
